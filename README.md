...
> effect is known**, so the four causal estimates the argument rests on are checked
> against ground truth — activation ATE, retention ITT, adoption ATT and DiD — including
...
cooperate. Normalized Qini is **0.020** on real data against **0.148** on
synthetic.
...
No external warehouse, no credentials. The pipeline itself makes no network calls;
two optional paths do — the AI summary layer below (off unless you ask for it) and
...
Every numeric token in the draft is extracted and checked against the fact sheet the
model was given, with one carve-out: bare small integers and round percentages
(0-10, 30, 50, 80, 90, 95, 100 and the two alphas) are allowed through, because they
show up as ordinary prose — "two co-primary metrics", "a 95% CI" — and would otherwise
fail every draft. The carve-out is why the fact sheet is scoped tightly: a fabricated
figure has to collide with an admissible *metric*, not with a stray small number. A number that cannot be traced back — a plausible-sounding
